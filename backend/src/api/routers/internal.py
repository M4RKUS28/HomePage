...
Flow:
  ...
  3. FastAPI validates credentials → returns **user data only** (no JWT)
  4. NextJS  → creates encrypted session cookie for the browser
  5. Every subsequent request: NextJS signs a fresh 30 s JWT and proxies
"""
...
from ..schemas.auth import InternalUserResponse, LoginRequest, RegisterRequest
...
@router.post("/login", response_model=InternalUserResponse)
async def internal_login(
    body: LoginRequest,
    db: AsyncSession = Depends(get_db),
):
    """
    Verify username + password and return **user data only**.

    Called by NextJS ``/api/auth/login`` route.  NextJS creates its
    own session - no JWT is minted here.
    """
    ...
    return InternalUserResponse(user=_build_user_read(user))
...
@router.post("/register", response_model=InternalUserResponse, status_code=status.HTTP_201_CREATED)
async def internal_register(
    body: RegisterRequest,
    db: AsyncSession = Depends(get_db),
):
    """
    Create a new user account and return **user data only**.
